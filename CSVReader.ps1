...
foreach($Class in $Classes)
{
    ...
    else
    {
        ...
        $CourseDue = $Class.Due
        $MeetingDetails = "$CourseNumber - $CourseName"
        $MeetingBody = "Lesson:  $CourseTopic `r`nDue:  $CourseDue"
        
    }
    write.host "Adding Calendar Item $MeetingDetails @ $CourseStart"
Add-CalendarMeeting -Subject $MeetingDetails -Body $MeetingBody -Location $Room -MeetingStart $CourseStart -MeetingDuration $ClassLength
}
